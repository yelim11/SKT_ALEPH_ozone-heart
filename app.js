(() => {
  ...
  function gemClass(source) {
    if (source === 'live') return 'gem-emerald';
    if (source === 'manual') return 'gem-rose';
    if (source === 'demo') return 'gem-violet';
  ...
  function applyGem(source) {
    ...
    resultGem.innerHTML = '<i class="facet facet-a"></i><i class="facet facet-b"></i><i class="facet facet-c"></i><i class="facet facet-d"></i>';
  }
  ...
  function showTempOnly(temp) {
    ...
    applyGem('tempOnly');
    resultCard.className = 'result-card state-temp-only';
  ...
  function showMeasured(temp, o3, source, detail = '') {
    ...
    applyGem(source);
    resultCard.className = `result-card state-${source}`;
  ...
  function checkInputs() {
    ...
    if (!validTemp(t)) {
      setError('기온을 -40~60°C 사이 숫자로 입력해 주세요.');
      temperature.focus();
      return;
    }
    if (o != null && !validOzone(o)) {
      setError('오존 값은 0~1000 µg/m³ 사이 숫자로 입력해 주세요.');
      ozone.focus();
      return;
    }

    if (o == null) showTempOnly(t);
    else showMeasured(t, o, 'manual', '직접 입력한 값입니다. 수치를 다시 확인하고 저장할 수 있습니다.');
  ...
  function persistHistory(items) {
    try {
      localStorage.setItem(STORAGE_KEY, JSON.stringify(items.slice(0, 7)));
      return true;
    } catch {
      return false;
    }
  ...
  function heartVars(source) {
    if (source === 'live') return ['#1bbf6b','#9ef1ba','#08733a'];
    if (source === 'manual') return ['#ef4384','#ffb0cd','#9f1e55'];
    if (source === 'demo') return ['#9948d9','#dcb1ff','#5a2287'];
    return ['#ef941f','#ffd26b','#9f4f10'];
  }
  ...
  function renderHistory() {
    ...
    for (const item of items) {
      ...
      const [h, hl, hd] = heartVars(item.source);
      const ozoneText = item.ozone == null ? '오존 미확인' : `오존 ${Number(item.ozone).toFixed(1)} µg/m³`;
      el.innerHTML = `
        <div class="history-mini" style="--h:${h};--hlight:${hl};--hdark:${hd}" aria-hidden="true"></div>
        <div class="history-meta">
          <strong>${Number(item.temperature).toFixed(1)}°C · ${ozoneText}</strong>
          <span>${fmtDate(item.timestamp)} · 기온 단독 예측 안 함</span>
        </div>
  ...
  function saveCurrent() {
    ...
    const ok = persistHistory(items);
    if (!ok) {
      sourceNote.textContent = '브라우저 저장 공간을 사용할 수 없어 기록 저장에 실패했습니다. 체크 기능은 계속 사용할 수 있습니다.';
      return;
    }
    renderHistory();
